...
namespace BuckshotPlusPlus.Compiler.HTML
{
    class Events
    {
		...
		public static string GetHTMLEvents(Token MyToken)
		{
			...
			{
				TokenDataVariable MyJSEventVar = TokenUtils.FindTokenDataVariableByName(ViewContainer.ContainerData, HTMLEvent.Name);
				if(MyJSEventVar != null)
                {
					Token MyJSEvent = TokenUtils.FindTokenByName(MyToken.MyTokenizer.FileTokens, MyJSEventVar.VariableData);
					if (MyJSEvent != null)
					{
						CompiledEvents += HTMLEvent.Name + "=\"" + JS.Event.GetEventString(MyJSEvent) + "\" ";
